...
export default function PendingApproval({ onNavigate }) {
  ...
  // Export the AI analysis report as a Word-compatible .doc file
  const exportReportToWord = (analysis, idea) => {
    if (!analysis) return;
    const scoreRows = [
      ['Creativity', analysis.creativity?.score],
      ['Market Demand', analysis.marketDemand?.score],
      ['Existing Solutions', analysis.existingSolutions?.score],
      ['Budget Feasibility', analysis.budgetFeasibility?.score],
      ['Timeline Feasibility', analysis.timelineFeasibility?.score],
      ['Scope Clarity', analysis.scopeClarity?.score],
      ['Risk Level (higher = safer)', analysis.riskLevel?.score],
      ['Overall Score', analysis.overallScore],
    ]
      .filter(([, s]) => s != null)
      .map(([label, s]) => `<tr><td>${label}</td><td style="text-align:center">${s}/10</td></tr>`)
      .join('');

    // HTML-escape helper — build entities via concatenation to survive formatters
    const E_AMP = '&' + 'amp;';
    const E_LT = '&' + 'lt;';
    const E_GT = '&' + 'gt;';
    const safeText = (t) => String(t || '')
      .replace(/&/g, E_AMP).replace(/</g, E_LT).replace(/>/g, E_GT);

    const reportHtml = safeText(analysis.report || 'No full report available.').replace(/\n/g, '<br/>');
    const recommendation = safeText(analysis.recommendation || 'N/A');
    const recommendationReason = safeText(analysis.recommendationReason || '');

    const html = `<!DOCTYPE html>
<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word">
<head>
  <meta charset="utf-8">
  <title>AI Innovation Idea Analysis Report</title>
  <!--[if gte mso 9]><xml><w:WordDocument><w:View>Print</w:View></w:WordDocument></xml><![endif]-->
  <style>
    body { font-family: 'Calibri', Arial, sans-serif; font-size: 11pt; color: #1f2937; line-height: 1.5; margin: 40px; }
    h1 { font-size: 18pt; color: #1e3a8a; border-bottom: 2px solid #1e3a8a; padding-bottom: 8px; }
    h2 { font-size: 14pt; color: #1e3a8a; margin-top: 24px; border-bottom: 1px solid #cbd5e1; padding-bottom: 4px; }
    ...
    .footer { margin-top: 30px; padding-top: 10px; border-top: 1px solid #cbd5e1; color: #6b7280; font-size: 9pt; }
    .reference { margin-top: 10px; color: #9ca3af; font-style: italic; font-size: 9pt; }
  </style>
</head>
<body>
  <h1>AI Innovation Idea Analysis Report</h1>
  <div class="meta">
    Idea ID: ${safeText(idea?.id || '')} &nbsp;|&nbsp; Idea: ${safeText(idea?.title || 'Untitled')} <br/>
    Analyzed: ${analysis.analyzedAt ? formatDateTime(analysis.analyzedAt) : '-'}
    &nbsp;|&nbsp; Provider: ${safeText(analysis.provider || '')}${analysis.model ? ' / ' + safeText(analysis.model) : ''}
  </div>

  <h2>1. Scorecard Overview</h2>
  <table>
    <tr><th style="width:70%">Dimension</th><th>Score</th></tr>
    ${scoreRows}
  </table>
  <p><strong>Recommendation:</strong> ${recommendation}${recommendationReason ? `<br/>${recommendationReason}` : ''}</p>

  <h2>2. Full Report</h2>
  <div class="report-text">${reportHtml}</div>

  <div class="footer">
    This report was generated by the I&D Hub AI analysis system.
  </div>
  <div class="reference">For reference only.</div>
</body>
</html>`;
    ...
    a.href = url;
    a.download = `AI-Report-${(idea?.id || 'idea').replace(/\//g, '-')}.doc`;
    a.click();
...
After completing the analysis above, output a STRICT JSON object (no markdown, no code fences) with the following keys for the scorecard:
{
  ...
  "summary": "...",
  "report": "THE COMPLETE FULL REPORT TEXT written according to the analysis instructions above. Include ALL sections: Executive Summary, Scorecard Overview with justifications, Detailed Analysis, AI Recommendations, and Appendices. Write it as a proper professional report in the tone requested."
}`;
  };
  ...
  const renderAiReport = (analysis) => {
    ...
    const { creativity, marketDemand, existingSolutions, budgetFeasibility, timelineFeasibility, scopeClarity, riskLevel, overallScore, recommendation, recommendationReason, summary, report, analyzedAt, provider, model } = analysis;
    ...
    return (
      <div className="ai-report">
        ...
        {report && (
          <div className="ai-report-full" style={{ marginTop: '1.5rem', padding: '1rem 1.25rem', backgroundColor: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            <h5 style={{ margin: '0 0 0.75rem', color: '#1e3a8a', fontWeight: 700 }}>
              📄 Full Report
            </h5>
            <div style={{ whiteSpace: 'pre-wrap', lineHeight: 1.6, color: '#334155', fontSize: '0.95rem' }}>
              {report}
            </div>
            <p style={{ marginTop: '1rem', color: '#9ca3af', fontStyle: 'italic', fontSize: '0.8rem', borderTop: '1px solid #e2e8f0', paddingTop: '0.5rem' }}>
              For reference only.
            </p>
          </div>
        )}
    ...
    <div className="page">
      ...
      <Modal isOpen={!!aiResult} onClose={closeAiModal} title="AI Analysis Result">
        ...
        <div className="modal-actions" style={{ marginTop: '1rem', justifyContent: 'space-between' }}>
          <button className="btn btn--secondary" onClick={() => exportReportToWord(aiResult, aiModal)}>
            📄 Export Word
          </button>
      ...
      <Modal isOpen={!!viewAiModal} onClose={closeViewAiModal} title={`AI Report - ${viewAiModal?.id || ''}`}>
        ...
        <div className="modal-actions" style={{ marginTop: '1rem', justifyContent: 'space-between' }}>
          {viewAiModal?.aiAnalysis && (
            <button className="btn btn--secondary" onClick={() => exportReportToWord(viewAiModal.aiAnalysis, viewAiModal)}>
              📄 Export Word
            </button>
          )}
